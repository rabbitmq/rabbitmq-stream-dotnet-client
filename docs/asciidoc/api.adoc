...
<5> The `ConfirmationHandler` defines an asynchronous callback invoked
...
make the `StreamSystem`, `Producer`, `Consumer` instances sluggish
or even block them. Any long processing should be done in
a separate thread (e.g. with an asynchronous `Task.Run(....)`).
====
...
====
You can mix different send methods. For example you can send a message with `send(Message)` and then send a batch of messages with `send(List<Message>)`.
...
on a given stream for this producer. 

Consider the `Reference` a logical name. It should not be a random sequence that
...
The `Reference` is only one part of the deduplication mechanism, the other part
is the _message publishing ID_. The publishing ID
is a strictly increasing sequence, starting at 0 and incremented for each message. 
...
the `DeduplicatingProducer#GetLastPublishedId()` method, which returns the last publishing ID for a given
...
A `Consumer` instance is created with `Consumer.Create(..)`. The main
...
consumer will stay idle, waiting for new messages to come in_. See the <<specifying-an-offset,offset section>> to find out more about the different
...
* `OffsetTypeOffset(offset)`: starting from the specified offset. 0 means consuming
...
* `OffsetTypeTimestamp(timestamp)`: starting from the messages stored
...
[source,java,indent=0]
--------
include::{test-examples}/ConsumerUsage.cs[tag=auto-tracking-defaults]
--------
...
[source,java,indent=0]
--------
include::{test-examples}/ConsumerUsage.cs[tag=auto-tracking-with-settings]
--------
<1> Set the consumer name (mandatory for offset tracking)
...
[source,java,indent=0]
--------
include::{test-examples}/ConsumerUsage.cs[tag=auto-tracking-only-with-name]
--------
...
[source,java,indent=0]
--------
include::{test-examples}/ConsumerUsage.cs[tag=manual-tracking-with-settings]
--------
<1> Set the consumer name (mandatory for offset tracking)
<2> Use manual tracking with defaults
...
[source,java,indent=0]
--------
include::{test-examples}/ConsumerUsage.cs[tag=subscription-listener]
--------
