...
<4> Subentry batch send is synchronous, there is not additional buffering. The messages are sent immediately. The messages are compressed and assigned to a single publishingid. 
It is useful when you want to compress the messages. For example logs or text messages.
The client by default supports GZIP algorithm or None. Other algorithms can be added by implementing the `ICompression` interface.
